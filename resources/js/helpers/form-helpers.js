...
function setCustomFileLabel() {
    ...
    inputFile.addEventListener("change", function () {
        if (!fileLabel) {
            throw new Error(
                `Adiciona classe '.js-custom-file-label' ao label do input file.`,
            );
        }
...
export function guideUserToTheFirstError() {
    ...
    const firstInvalidInputOffsetTop =
    ...
    function _scrollToError(container) {
        const animateConfig = {
            top:
                firstInvalidInputOffsetTop +
                currentScrollPosition -
                _getStickyHeaderOffset(),
            behavior: "smooth",
        };
    ...
    function _getStickyHeaderOffset() {
        const stickyHeaderSticky = document.querySelector(
            ".fmd-header-is-fixed",
        );
        return stickyHeaderSticky
            ? stickyHeaderSticky.getBoundingClientRect().height
            : 0;
    }
}
