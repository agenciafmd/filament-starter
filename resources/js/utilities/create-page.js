...
function createBladePHPPage(pageName) {
    ...
    const templateFile = fs.readFileSync(
        "packages/agenciafmd/frontend/src/resources/views/html/create-page-template.blade.php",
        "utf8",
    );
    const pageTemplate = templateFile.replaceAll(
        "<!--fmd:pageName-->",
        pageName,
    );
...
function createStylePage(stylePageName) {
    ...
    childProcess.exec(
        `touch ${pagePath} && ${importNewStylePageInFrontendSCSS}`,
        function (error, stdout) {
            ...
            console.log(
                `✅ ${stylePageName}.scss criada e importada com sucesso.`,
            );
        },
    );
}
