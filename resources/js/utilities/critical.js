...
function dataToBeReplacedInWebpack() {
    const webpackFile = fs.readFileSync(webpackPath, {
        encoding: "utf8",
        flag: "r",
    });
    const URLsToReplace = webpackFile.substring(
        webpackFile.search("urls:"),
        webpackFile.search("// fmd:criticalPath"),
    );
...
fs.writeFile(webpackPath, dataToBeReplacedInWebpack(), (err) => {
    ...
    console.log(
        `✅ As páginas do /HTML foram adicionadas ao ${webpackPath} com sucesso.`,
    );
});
