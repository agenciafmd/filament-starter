...
async function generateManifest() {
    try {
        ...
        console.log(
            `✅  PWA manifest successfully generated! File path: ${outputPath}`,
        );
    } catch (error) {
        console.error(
            `⚠️  Oops! Something went wrong while generating the manifest: ${error.message}`,
        );
    }
}
...
function getViteManifest() {
    ...
    if (!existsSync(manifestPath)) {
        throw new Error(
            `Manifest não encontrado em ${manifestPath}. Certifique-se de que o Build tenha sido executado.`,
        );
    }
...
function getStringInFile(file, keyword, initialSelector, finalSelector) {
    ...
    lines.forEach((line) => {
        if (line.includes(keyword)) {
            ...
            const indexOfHexaEnd = line.indexOf(
                finalSelector,
                indexOfHexaInitial,
            );

            occurrences.push(
                line.substring(indexOfHexaInitial, indexOfHexaEnd).trim(),
            );
        }
    });
