...
let HTMLPagesFiles = fs.readdirSync(
    "packages/agenciafmd/frontend/src/resources/views/html",
);
...
function formatPageArrayFiles() {
    ...
    HTMLPagesFiles = HTMLPagesFiles.map((page) =>
        page.replace(".blade.php", ""),
...
function generateReports(environment) {
    return new Promise((resolve) => {
        ...
        const createLighthouseReportFolder =
            "rm -rf ./public/web-vitals/ && mkdir -p ./public/web-vitals";
        const createLighthouseReportJSONFolder =
            "rm -rf ./resources/web-vitals/ && mkdir -p ./resources/web-vitals";

        childProcess.exec(
            `${createLighthouseReportFolder} && ${createLighthouseReportJSONFolder}`,
                            ...
                            (error, data) => {
                                ...
                                const reportComplete = {
                                    ...
                                    categories: {
                                        ...
                                        seo: {
                                            label: "SEO",
                                            score: scoreStatus(report.seo.score)
                                                .score,
                                            status: scoreStatus(
                                                report.seo.score,
                                            ).status,
                                        ...
                                        pwa: {
                                            label: "PWA",
                                            score: scoreStatus(report.pwa.score)
                                                .score,
                                            status: scoreStatus(
                                                report.pwa.score,
                                            ).status,
...
environmentsPromptQuestion.then((environments) => {
    ...
    if (environmentsSelected.includes("Local")) {
        generateReports(environmentsSetup.local).then((resolveReports) => {
            fs.writeFile(
                `./resources/web-vitals/web-vitals-${environmentsSetup.local.id}.json`,
                JSON.stringify(resolveReports),
                (err) => {
                    if (err) {
                        console.log(err);
                        return;
                    ...
                    console.log(
                        `✅ Reportes de ${environmentsSetup.local.domain} adicionados com sucesso.`,
                    );
                    allReportsPerPage = [];
                },
            );
    ...
    if (environmentsSelected.includes("Homologação")) {
        ...
        homologationURLPromptQuestion.then((homologationURL) => {
            const sanitazedURL =
                homologationURL.homologationURLInserted.endsWith("/")
            ...
            generateReports(environmentsSetup.homologation).then(
                (resolveReports) => {
                    fs.writeFile(
                        `./resources/web-vitals/web-vitals-${environmentsSetup.homologation.id}.json`,
                        JSON.stringify(resolveReports),
                        (err) => {
                            if (err) {
                                console.log(err);
                            ...
                            console.log(
                                `✅ Reportes de ${environmentsSetup.homologation.domain} adicionados com sucesso.`,
                            );
                            allReportsPerPage = [];
                        },
                    );
